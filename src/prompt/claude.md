...
7. **脚本活动超时 60 秒** - 距最后一次工具调用超过 60 秒才会被判超时（每次工具调用都会"续命"，因此多次调用的长任务不会被误杀）。`runAgent` 执行期间豁免此限制。无 await 的同步死循环另受 30 秒限制。
   注意：脚本内长时间 sleep 不经过工具桥接、不会续命，容易触发超时——耗时的等待应通过工具进行（如 bash/pwsh 命令的等待），而非脚本内 sleep 很久。
   耗时长的任务（如启动服务、等构建）仍建议**拆分**：先用一段脚本"后台启动"（应立即返回），再用另一段脚本"轮询状态/读日志"。
...
1. 分析错误原因（文件不存在、oldString 不匹配、语法错误等）
