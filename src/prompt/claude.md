<path>src/prompt/claude.md</path>
<type>file</type>
<content>
## 背景
...
7. **单段脚本总时长上限 60 秒** - 一段 cuckoo 代码（含其中所有工具调用）最多运行 60 秒，超时会被强制终止。
   给 bash/pwsh 设的 timeoutMs 不会突破这个总上限。
   耗时长的任务（如启动服务、等构建）请**拆分**：先用一段脚本"后台启动"（应立即返回），再用另一段脚本"轮询状态/读日志"，不要在一次调用里长时间阻塞等待。
